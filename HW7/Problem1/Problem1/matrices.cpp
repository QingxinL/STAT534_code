...
/*
double* selectAcolumn(gsl_matrix* M, int d)
{
    double column[M->size1];
    for (int i=0; i<M-size1)
    {
        column[i] = gsl_matrix_get_col(<#gsl_vector *v#>, <#const gsl_matrix *m#>, <#const size_t j#>)
    }
    return (column);
